...
/**
 * Extra portfolio safety checks that should hold regardless of UI / workflow.
 *
 * These tests use SQLite in-memory so they can run without external DB.
 */
class PortfolioEdgeCasesTest extends TestCase
{
    public function testSellWithoutAnyBuyDoesNotCreateNegativePosition(): void
    {
        ...
        // Regardless of whether the service rejects the trade or not,
        // we must never end up with a negative position.
        $pos = DB::table('portfolio_positions')->where('account_id', 1)->where('ticker_id', 1)->first();
        if ($pos) {
            $this->assertGreaterThanOrEqual(0, (int)$pos->qty, 'Position qty must never be negative');
        }

        // It is acceptable for the service to reject this (preferred).
        // If rejected, ok should be false.
        if (isset($r['ok']) && $r['ok'] === false) {
            $this->assertTrue(true);
        }
    }

    public function testOversellDoesNotCreateNegativeLotsOrMatchesBeyondBuys(): void
    {
        $this->bootSqliteMemory();
        $this->createMinimalPortfolioSchema();
        ...
        $this->assertTrue($b['ok'], 'BUY failed: ' . json_encode($b));
        ...
        $lots = DB::table('portfolio_lots')->get()->all();
        foreach ($lots as $lot) {
        ...
        $this->assertLessThanOrEqual(100, $matched, 'Matched qty must never exceed total buys');

        $pos = DB::table('portfolio_positions')->where('account_id', 1)->where('ticker_id', 1)->first();
        if ($pos) {
            $this->assertGreaterThanOrEqual(0, (int)$pos->qty);
        }

        // If the service rejects oversell, ok should be false.
        // If it allows but clamps, ok may be true; either way invariants above must hold.
        $this->assertIsArray($s);
    }

    public function testValueEodOnTradingDayUsesSameTradeDate(): void
    {
        $this->bootSqliteMemory();
        $this->createMinimalPortfolioSchema();
        ...
        $vr = $svc->valueEod('2026-01-27', 1);
        $this->assertTrue($vr['ok'], 'valueEod failed: ' . json_encode($vr));

        $pos = DB::table('portfolio_positions')->where('account_id', 1)->where('ticker_id', 1)->first();
        $this->assertNotNull($pos);
        $this->assertSame('2026-01-27', (string)$pos->last_valued_date);
    }
    ...
    private function createMinimalPortfolioSchema(): void
    {
        ...
        Schema::create('portfolio_lot_matches', function ($t) {
            ...
            $t->integer('account_id');
            $t->integer('ticker_id');
            // Keep column order consistent with repositories.
            $t->integer('sell_trade_id');
            $t->integer('buy_lot_id');
            $t->integer('matched_qty');
            // These names are used by the portfolio repositories/service.
