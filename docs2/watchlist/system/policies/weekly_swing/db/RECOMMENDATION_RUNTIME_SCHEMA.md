...
- (`trade_date`, `ticker`, `capital_mode`, `policy_code`, `paramset_version`) if policy/version context is stored externally; or
- include `policy_code` and `paramset_version` directly in the artifact table if persistence needs stronger replay traceability.
