...
> - `8.strategi.md` hanya dokumentasi langkah operasional (runbook) untuk operator/user.
...
- Jika tabel/row status tidak tersedia untuk hari eksekusi, watchlist **tetap jalan** dengan default **REGULAR**.
  - Rujukan contract: `docs/watchlist/1.contract.md` bagian *Data Degradation Rules* (reason: `{code:"GL_TICKER_STATUS_DEFAULTED_REGULAR", severity_level:"WARN"}`).
...
Output:
- strict preopen contract sesuai `docs/watchlist/3.preopen.md`.
...
Tujuan: membatasi loop DELAY dan menegakkan cooldown/window sesuai `docs/watchlist/4.scorecard.md` (Retry budget & cooldown).

Rujukan format snapshot JSON & result ada di `docs/watchlist/4.scorecard.md`.
