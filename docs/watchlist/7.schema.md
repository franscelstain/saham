...
> - `7.schema.md` hanya dokumentasi untuk membantu pembaca memahami struktur tersebut.
...
- `confirm_retry_count` (INT, default 0): jumlah DELAY yang sudah terjadi untuk ticker ini pada `trade_date` (lihat `docs/watchlist/4.scorecard.md` bagian Retry budget).
...
- `checked_at` (DATETIME)
- `snapshot_json` (JSON): input live snapshot (sesuai docs/watchlist/4.scorecard.md)
...
- Manual (SQL insert) untuk tahap awal/POC (lihat `docs/watchlist/8.strategi.md`).
