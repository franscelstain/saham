...
Harga per tranche (plan_limit_price / plan_price_cap) **tidak** ditentukan oleh policy; wajib mengikuti kontrak global di `docs/watchlist/2.watchlist.md` (EOD_TRANCHE_PRICE_INTENT_V1). Policy hanya menentukan `mode`, `tranche_pct`, dan timing/condition.
...
Policy ini memakai rule global **EOD_TRANCHE_RULE_V1** (lihat `docs/watchlist/2.watchlist.md` → Mini strategy).
