...
> - Universe filter + grouping + recommendations allocation ada di `2.watchlist.md` (global).
> - CONFIRM checks ada di `4.scorecard.md`.

Dokumen ini hanya mendefinisikan aturan spesifik policy. Aturan universal wajib lihat `2.watchlist.md` bagian **Global Contract**.

Kontrak lintas-policy ada di `2.watchlist.md`.
...
**Jika gagal Hard Rules di bawah ini:** ticker tetap boleh tampil untuk monitoring, tetapi statusnya **NOT_QUALIFIED** → `plan.is_eligible_new_entry=false` dan masuk group `watch_only` (lihat `2.watchlist.md`).
...
Harga per tranche (plan_limit_price / plan_price_cap) **tidak** ditentukan oleh policy; wajib mengikuti kontrak global di `2.watchlist.md` (EOD_TRANCHE_PRICE_INTENT_V1). Policy hanya menentukan `mode`, `tranche_pct`, dan timing/condition.
...
Policy ini memakai rule global **EOD_TRANCHE_RULE_V1** (lihat `2.watchlist.md` → Mini strategy).
