...
Kontrak:
- Ticker dianggap relevan jika policy menghasilkan minimal satu “marker” (contoh: sinyal/pattern/momentum/volume trigger) pada as-of EOD date.
...
**Jika gagal Hard Rules di bawah ini:** ticker tetap boleh tampil untuk monitoring, tetapi statusnya **NOT_QUALIFIED** → `plan.is_eligible_new_entry=false` dan masuk group `watch_only` (lihat `docs/watchlist/2a.group_semantics.locked.md`).
