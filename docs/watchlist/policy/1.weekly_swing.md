...
- Ticker dianggap relevan jika pada **salah satu** dari **N hari bursa terakhir** (termasuk `as-of EOD date`) memiliki `signal_code != 0`.
...
**Jika gagal Hard Rules di bawah ini:** ticker tetap boleh tampil untuk monitoring, tetapi statusnya **NOT_QUALIFIED** → `plan.is_eligible_new_entry=false` dan masuk group `avoid` (lihat `docs/watchlist/2a.group_semantics.locked.md`).
