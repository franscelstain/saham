...
Dokumen ini adalah **CONTRACT** untuk CONFIRM (intraday/live) membandingkan PLAN vs kondisi real-time.

- Struktur `reasons[]` dan severity levels mengikuti **`docs/watchlist/1.contract.md`** (jangan didefinisikan ulang di sini).
- CONFIRM **tidak boleh mengubah PLAN** (PLAN immutable).
- Universe CONFIRM hanyalah ticker yang sudah ada di output Watchlist.
...
- `reasons[]` (CF_*) + `inputs_used` (ask1/bid1/spread/snapshot_age) (array of **CandidateReason** objects `{ code, message, severity_level }`, lihat `docs/watchlist/1.contract.md`)
...
- Enable/disable per policy via `trade.watchlist.scorecard.policy_overrides[POLICY]`
  - `depth_top_n`: `3` or `5` enables the guard; `0` disables it.
  - `min_depth_lots`: shortcut to set the same minimum lots for both sides.
  - `min_depth_lots_bid` / `min_depth_lots_ask`: per-side overrides (optional).

If you don't override thresholds, defaults follow the values above (2,000 for N=3; 3,000 for N=5).
