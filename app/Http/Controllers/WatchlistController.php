...
class WatchlistController extends Controller
{
    ...
    public function preopen()
    {
	    ...
	    // Strict contract docs/watchlist/preopen.md uses `capital_idr` and `asof_eod_date`.
	    // Support backward compatible aliases (`capital_total`, `capital`, `trade_date`).
	    $capital = request()->query('capital_idr', request()->query('capital_total', request()->query('capital')));
	    ...
	    $opts = [
	        ...
	        'eod_date' => request()->query('asof_eod_date') ? (string) request()->query('asof_eod_date') : (request()->query('trade_date') ? (string) request()->query('trade_date') : null),
