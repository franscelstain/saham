        ...
        if ($capitalTotal !== null && $capitalTotal > 0) {
            ...
            foreach ($split as $s) {
                ...
                $plan[] = [
                    ...
                    'dv20' => isset($row['dv20']) && is_numeric($row['dv20']) ? (float)$row['dv20'] : null,
                    'liq_bucket' => (string)($row['liq_bucket'] ?? ''),
                    'candle_body_pct' => isset($row['candle_body_pct']) && is_numeric($row['candle_body_pct']) ? (float)$row['candle_body_pct'] : null,
                    'candle_upper_wick_pct' => isset($row['candle_upper_wick_pct']) && is_numeric($row['candle_upper_wick_pct']) ? (float)$row['candle_upper_wick_pct'] : null,
                    'candle_lower_wick_pct' => isset($row['candle_lower_wick_pct']) && is_numeric($row['candle_lower_wick_pct']) ? (float)$row['candle_lower_wick_pct'] : null,
                    'is_inside_day' => isset($row['is_inside_day']) ? (bool)$row['is_inside_day'] : null,
                    'engulfing_type' => (string)($row['engulfing_type'] ?? ''),
                    'is_long_upper_wick' => isset($row['is_long_upper_wick']) ? (bool)$row['is_long_upper_wick'] : null,
                    'is_long_lower_wick' => isset($row['is_long_lower_wick']) ? (bool)$row['is_long_lower_wick'] : null,

                    'decision_code' => isset($row['decision_code']) ? (int)$row['decision_code'] : null,
                    'signal_code' => isset($row['signal_code']) ? (int)$row['signal_code'] : null,
                    'volume_label_code' => isset($row['volume_label_code']) ? (int)$row['volume_label_code'] : null,
                    'dv20' => isset($row['dv20']) && is_numeric($row['dv20']) ? (float)$row['dv20'] : null,
                    'liq_bucket' => (string)($row['liq_bucket'] ?? ''),
                    'candle_body_pct' => isset($row['candle_body_pct']) && is_numeric($row['candle_body_pct']) ? (float)$row['candle_body_pct'] : null,
                    'candle_upper_wick_pct' => isset($row['candle_upper_wick_pct']) && is_numeric($row['candle_upper_wick_pct']) ? (float)$row['candle_upper_wick_pct'] : null,
